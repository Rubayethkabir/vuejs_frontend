  ...
  <div class="listBody">
    <div class="container mt-5 mb-5">
      ...
      <div class="row g-2" v-if="!isLoading">
        <div class="col-md-3" v-for="(item, index) in doctorList" :key="index">
          <div class="card p-2 py-3 text-center">
            ...
            <small>{{ item.doctorZone ? item.doctorZone : "N/A" }}</small>
            ...
            <div class="mt-4 apointment">
              <router-link
                :to="{ path: `/doctor/details/${item.id}` }"
                exact
                class="nav-link"
              >
  ...
  methods: {
    handleSearch() {
      ...
      if (this.searchValue.length > 0) {
        console.log(this.searchValue);
        this.$store.dispatch("doctor/getDoctorListByName", this.searchValue);
      } else {
        this.$store.dispatch("doctor/getDoctorList", this.doctorGroup);
      }
    }
  },
  mounted() {
    this.doctorGroup = this.$route.params.doctorGroup;
    this.$store.dispatch("doctor/getdoctorList", this.doctorGroup);
